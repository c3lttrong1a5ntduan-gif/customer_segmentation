...
class FeatureEngineer:
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        # 1. Đọc self.data_path linh hoạt theo đuôi file
        ...
        # 2. Đọc self.raw_data_path linh hoạt theo đuôi file
        if self.raw_data_path and os.path.exists(self.raw_data_path):
            if str(self.raw_data_path).lower().endswith(('.xlsx', '.xls')):
                self.raw_df = pd.read_excel(self.raw_data_path, engine="openpyxl")
            else:
    ...
    def _compute_lifecycle(
        ...
        if self.raw_df is not None:
            ...
            agg_count = inv_flags.groupby("CustomerID").agg(
            n_cancelled=("is_cancelled", "sum"), n_total=("is_cancelled", "count")
            )
